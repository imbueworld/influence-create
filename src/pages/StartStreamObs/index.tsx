    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                ...
                streamData = await CryptoJS.AES.decrypt(streamData, event._name).toString(
                    CryptoJS.enc.Utf8
                );
                ...
                timer.current = window.setInterval(() => getConnected(decryptedStreamId), 5000);
                setLoading(false);
            } catch (error) {
                console.log(error);
            }
                ...
                {loading ? (
                    ...
                    <>
                        <div className="absolute w-full top-[90%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 grid grid-cols-6 items-center">
                            <div className="bg-[#DEFCFC] rounded-lg ml-2 mb-10">
                                <div className=""><h5 className="text-[9px]" >RTMP URL<br /></h5><p className="text-[9px] break-words lowercase cursor-pointer pb-3" onClick={() => {
                                    navigator.clipboard.writeText("rtmp://rtmp.livepeer.com/live/")
                                    alert("Url Copied")
                                }}>{"rtmp://rtmp.livepeer.com/live/"}</p></div>
                                <div><h5 className="text-[9px] ">Stream Key<br /></h5><p  onClick={() => {
                                    navigator.clipboard.writeText(streamKey)
                                    alert("Stream key Copied")
                                }}
                                    className="text-[7px] lowercase cursor-pointer pb-3">{streamKey}</p></div>
                            </div>
